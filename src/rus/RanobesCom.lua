-- {"id":962041,"ver":"1.0.15","libVer":"1.0.0","author":"MysterioCrypto","dep":[]}
...
local function pageFromData(data)
    if data and PAGE and data[PAGE] then return tonumber(data[PAGE]) or 1 end
    if data and data["page"] then return tonumber(data["page"]) or 1 end
    return 1
end

local function delaySafe(isSearch)
    if isSearch then
...
local function safeFetch(url, soft)
    ...
    if title == "Error" or title == "Ranobes Flood Guard" or title == "Just a moment..." or title:find("Антибот") or body:find("Ranobes Flood Guard") or body:find("подозрительную активность") then
        if soft then return nil end
...
local function imageFromStyle(style)
    local url = trim(style):match("url%((.-)%)")
    if not url or url == "" then return "" end
    url = trim(url):gsub("^['\"]", ""):gsub("['\"]$", "")
    if url == "" then return "" end
    return normalizeURL(url)
end

local function cardImage(card)
    local fig = first(card, { "figure.cover", ".cover", "figure" })
    local url = imageFromStyle(attrOf(fig, "style"))
...
local function findCardTitleLink(card)
    ...
    for i = 1, links:size() do
        ...
        if isNovelHref(attrOf(link, "href")) and textOf(link) ~= "" then
            return link
        end
    end
    return nil
...
local function addNovel(out, seen, title, href, img)
...
local function parseCards(root)
    local out = {}
    local seen = {}
...
local function parseNovel(novelURL, loadChapters)
    ...
    if loadChapters then
        ...
        if firstDoc then
            ...
            if total <= MAX_CHAPTER_PAGES then
                pages[1] = firstDoc
            else
                fromPage = total - MAX_CHAPTER_PAGES + 1
            end

            for p = fromPage, total do
                if not pages[p] then
                    delaySafe(false)
                    local pageDoc = safeFetch(indexURL .. "page/" .. p .. "/", true)
                    if pageDoc then pages[p] = pageDoc end
                end
            end

            for p = total, fromPage, -1 do
                if pages[p] then
