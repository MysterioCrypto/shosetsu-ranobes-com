-- {"id":962041,"ver":"1.0.9","libVer":"1.0.0","author":"MysterioCrypto","dep":[]}
...
local function shrinkURL(url)
    ...
    if startsWith(url, baseURL) then return url:sub(#baseURL + 1) end
    return url
end
...
local function first(root, selectors)
    ...
    for _, selector in ipairs(selectors) do
        local n = root:selectFirst(selector)
...
local function randomizedDelay(isSearch)
    ...
    if isSearch then
        ...
        if consecutiveTriggers <= 2 then d = math.random(900, 1700) else d = math.random(2500, 4200) end
    else
        d = math.random(1500, 3000)
    end
    delay(d)
end
...
local function imageFromStyle(style)
    style = trim(style)
    local u = style:match("url%((.-)%)")
    if not u or u == "" then return "" end
    u = trim(u):gsub("^['\"]", ""):gsub("['\"]$", "")
    if u ~= "" then return normalizeURL(u) end
    return ""
end

local function cardImage(card)
    local fig = first(card, { "figure.cover", ".cover", ".poster-img" })
...
local function buildCatalogURL(data)
    local p = pageFromData(data)
    if p <= 1 then return baseURL .. "/ranobe/" end
    return baseURL .. "/ranobe/page/" .. p .. "/"
end
...
local function parseCatalogURL(url, withDebug)
    ...
    local out, cardCount = parseCards(root, "")
    if #out > 0 then return out, nil end
    local reason = "no catalog novels: url=" .. tostring(url) .. "; cards=" .. tostring(cardCount) .. "; title=" .. textOf(doc:selectFirst("title"))
    if withDebug then return makeDebugNovel("no catalog novels", reason) end
    return {}, reason
end
...
local function parseSearchURL(url, query, withDebug)
    ...
    local root = doc:selectFirst("#dle-content") or doc:selectFirst("main") or doc
    local out, cardCount = parseCards(root, query or "")
    if #out > 0 then return out, nil end
    local reason = "no search novels: url=" .. tostring(url) .. "; cards=" .. tostring(cardCount) .. "; title=" .. textOf(doc:selectFirst("title"))
    if withDebug then return makeDebugNovel("no search novels", reason) end
    return {}, reason
end

local function parseListingURL(url)
    return parseCatalogURL(url, true)
end

local function search(data)
    local raw = queryFromData(data)
    if raw == "" then return makeDebugNovel("empty query", "Shosetsu did not pass QUERY to extension") end
    local novels, reason = parseSearchURL(searchURL(raw, pageFromData(data)), raw, false)
...
local function mapStatus(s)
    s = trim(s)
    return ({
        ["Активен"] = NovelStatus.PUBLISHING, ["Активно"] = NovelStatus.PUBLISHING, ["В процессе"] = NovelStatus.PUBLISHING, ["Продолжается"] = NovelStatus.PUBLISHING, ["Онгоинг"] = NovelStatus.PUBLISHING,
        ["Ongoing"] = NovelStatus.PUBLISHING, ["Active"] = NovelStatus.PUBLISHING,
        ["Завершено"] = NovelStatus.COMPLETED, ["Завершён"] = NovelStatus.COMPLETED, ["Завершена"] = NovelStatus.COMPLETED, ["Закончен"] = NovelStatus.COMPLETED, ["Закончено"] = NovelStatus.COMPLETED, ["Completed"] = NovelStatus.COMPLETED,
        ["Приостановлено"] = NovelStatus.PAUSED, ["Заморожено"] = NovelStatus.PAUSED, ["Пауза"] = NovelStatus.PAUSED, ["Break"] = NovelStatus.PAUSED, ["Hiatus"] = NovelStatus.PAUSED
...
local function findSpecValue(doc, labels)
    local nodes = doc:select("div.r-fullstory-spec li")
    if nodes:size() == 0 then nodes = doc:select(".r-fullstory-spec li") end
...
local function parseNovel(novelURL, loadChapters)
    ...
    if loadChapters then
        local chapters = {}
        local indexURL = ensureTrailingSlash(findChapterIndexUrl(doc, fullURL))
        ...
        if firstDoc then
            ...
            for p = total, 1, -1 do
                if pages[p] then chapters = concatLists(chapters, parseChapters(pages[p])) end
            end
        end
