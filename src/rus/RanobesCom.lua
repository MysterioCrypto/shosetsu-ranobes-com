...
local function randomizedDelay(isSearch)
    local d
    if isSearch then consecutiveTriggers = consecutiveTriggers + 1; if consecutiveTriggers <= 2 then d = math.random(900, 1700) else d = math.random(2500, 4200) end
    else d = math.random(1500, 3000) end
    delay(d)
end
...
local function queryMatches(title, link, query)
    query = trim(query); if query == "" then return true end
...
local function safeFetch(url, soft)
    ...
    if not ok then local err = tostring(document); local code = err:match("(%d%d%d)"); if soft then return false, code or "unknown", err end; error("HTTP error: " .. (code or err)) end
...
local function styleImageURL(node) if not node then return "" end; local u = attrOf(node, "style"):match("url%(['\"]?(.-)['\"]?%)"); if not u or u == "" then return "" end; return normalizeURL(u) end
local function cardImage(card)
    local u = styleImageURL(first(card, { ".cover", "figure", ".poster", ".poster-img" })); if u ~= "" then return u end
    local img = first(card, { "img[data-src]", "img[src]" }); local src = attrOf(img, "data-src"); if src == "" then src = attrOf(img, "src") end; if src ~= "" then return normalizeURL(src) end
    return imageURL
end
...
local function findSpecValue(doc, labels)
    ...
    for i = 1, nodes:size() do local li = nodes:get(i - 1); local txt = textOf(li); for _, label in ipairs(labels) do if txt:find(label) then local v = textOf(first(li, { "span a", "span" })); if v ~= "" then return v end; return trim(txt:gsub(label .. "%s*: ?", "")) end end end
    return ""
end
...
local function findChapterIndexUrl(doc, novelURL)
    ...
    local slug = shrinkURL(novelURL):match("/ranobe/%d+%-([^/%.]+)%.html"); if slug and slug ~= "" then return baseURL .. "/chapters/" .. slug .. "/" end
    error("Chapter index URL not found.")
end
local function getLastPage(doc) local max = 1; local links = doc:select("a[href*='/page/']"); for i = 1, links:size() do local p = tonumber(attrOf(links:get(i - 1), "href"):match("/page/(%d+)/?")); if p and p > max then max = p end end; return max end
...
local function parseNovel(novelURL, loadChapters)
    ...
    if loadChapters then local chapters = {}; local indexURL = ensureTrailingSlash(findChapterIndexUrl(doc, fullURL)); local firstDoc = safeFetch(indexURL, true); if firstDoc then local total = getLastPage(firstDoc); local pages = { firstDoc }; for p = 2, total do randomizedDelay(false); local pd = safeFetch(indexURL .. "page/" .. p .. "/", true); if pd then pages[p] = pd end end; for p = total, 1, -1 do if pages[p] then chapters = concatLists(chapters, parseChapters(pages[p])) end end end; info:setChapters(AsList(chapters)) end
    return info
end
